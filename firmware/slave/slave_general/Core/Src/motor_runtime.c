        ...
        switch (motors_rt[i].state) {
            ...
            case MOTOR_ZEROING: {
                ...
                if (abs_pos < MOTOR_ZERO_TOL) {
                    /* Arrived — lock in ARMED_HOLD at zero */
...
{
    if (idx >= N_MOTORS) return HAL_ERROR;
    if (motors_rt[idx].state == MOTOR_ARMED_HOLD ||
        motors_rt[idx].state == MOTOR_ARMED_MIT) {
...
HAL_StatusTypeDef motor_runtime_goto_zero(uint8_t idx, uint16_t cmd_seq)
{
    ...
    if (mz != NULL) motors_rt[idx].pos = mz->pos;
