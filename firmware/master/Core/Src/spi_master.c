...
static uint8_t slave_state_byte = 0;
...
/* --- Core SPI Transfer --- */
static HAL_StatusTypeDef spi_send_to_one_slave(SpiDevId dev, uint8_t cmd, uint8_t num_motors, motor_cmd_t *cmds, motor_cmd_t *feedbacks) {
    uint8_t tx_buf[1 + MAX_MOTORS_PER_SLAVE * BYTES_PER_MOTOR] = {0};
    uint8_t rx_buf[1 + MAX_MOTORS_PER_SLAVE * BYTES_PER_MOTOR] = {0};
    uint16_t total = (uint16_t)(1 + num_motors * BYTES_PER_MOTOR);

    if (num_motors > MAX_MOTORS_PER_SLAVE || num_motors == 0) return HAL_ERROR;

    tx_buf[0] = cmd;
    for (uint8_t i = 0; i < num_motors; i++) {
        pack_one_motor(&tx_buf[1 + i * BYTES_PER_MOTOR], &cmds[i]);
    }

    CS_SELECT(dev);
    HAL_StatusTypeDef st = HAL_SPI_TransmitReceive(master_hspi, tx_buf, rx_buf, total, HAL_MAX_DELAY);
    CS_ALL_HIGH();

    if (st == HAL_OK) {
        slave_state_byte = rx_buf[0];
        for (uint8_t i = 0; i < num_motors; i++) {
            unpack_one_motor_feedback(&rx_buf[1 + i * BYTES_PER_MOTOR], &feedbacks[i]);
        }
    }
...
void MotorMaster_ProcessLoop(void) {
    ...
    if ((int32_t)(now - next_poll_ms) >= 0) {
        ...
        if (send_disarm) {
            ...
            cmd = first_arm_packet ? SPI_CMD_ARM : SPI_CMD_HOLD;
            first_arm_packet = 0;
        } else {
            cmd = SPI_CMD_NOP;
        }

        HAL_StatusTypeDef st = spi_send_to_one_slave(
            slaves[0].slv_id,
            cmd,
            slaves[0].active_motor_count,
            slaves[0].slv_motor_cmds,
            slaves[0].slv_motor_feedbacks);

        if (st == HAL_OK) {
            usb_printf("motor=%u pos=%.4f state=%s\r\n",
                       (unsigned)slaves[0].slv_motor_feedbacks[0].motor_id,
                       slaves[0].slv_motor_feedbacks[0].position,
                       slave_state_byte ? "ARMED" : "IDLE");
        }
    }
}
